...
(function (global, factory) {
    ...
    c3_chart_internal_fn.updateEventRect = function (eventRectUpdate) {
        ...
        eventRectUpdate.attr('class', $$.classEvent.bind($$)).select('rect').attr("x", x).attr("y", y).attr("width", w).attr("height", h);
    };
    c3_chart_internal_fn.generateEventRectsForSingleX = function (eventRectEnter) {
        var $$ = this,
            d3 = $$.d3,
            config = $$.config;
        eventRectEnter.append("a").attr("class", $$.classEvent.bind($$)).attr("xlink:href", '').attr("aria-describedby", $$.tooltipId).style("cursor", config.data_selection_enabled && config.data_selection_grouped ? "pointer" : null).on('mouseover', function (d) {
            var index = d.index;
    ...
    c3_chart_internal_fn.generateEventRectsForMultipleXs = function (eventRectEnter) {
        ...
        eventRectEnter.append('a').attr('class', CLASS.eventRect).attr('xlink:href', '').attr("aria-describedby", $$.tooltipId).on('mouseout', function () {
            if (!$$.config) {
                return;
            } // chart is destroyed
            if ($$.hasArcType()) {
            ...
            $$.dragend();
        }) : function () {}).append('rect').attr('x', 0).attr('y', 0).attr('width', $$.width).attr('height', $$.height);
    };
    ...
    c3_chart_internal_fn.updateTargetsForLine = function (targets) {
        ...
        mainLineEnter = mainLineUpdate.enter().append('g').attr('class', classChartLine).attr('role', 'graphics-object group').style('opacity', 0).style("pointer-events", "none");
        // Lines for each data
    ...
    c3_chart_internal_fn.updateCircle = function () {
        ...
        $$.mainCircle.enter().append("circle").attr("role", "graphics-symbol img").attr("class", $$.classCircle.bind($$)).attr("r", $$.pointR.bind($$)).style("fill", $$.color);
